...
import 'package:canteen_food_ordering_app/screens/controlHome.dart';
import 'package:canteen_food_ordering_app/screens/chefHome.dart';
import 'package:canteen_food_ordering_app/screens/login.dart';
import 'package:canteen_food_ordering_app/screens/formHome.dart';
  ...
  try {
    if (authResult != null) {
      ...
      if (!firebaseUser.isEmailVerified) {
        ...
        } else if (authNotifier.userDetails.role == 'user') {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (BuildContext context) {
              return ChefHomePage();
...
insertedchefpage(BuildContext context) async {
  pr = new ProgressDialog(context,
      type: ProgressDialogType.Normal, isDismissible: false, showLogs: false);
  pr.show();
  //AuthResult authResult;
...
submitformm(Formm user, AuthNotifier authNotifier, BuildContext context) async {
  pr = new ProgressDialog(context,
      type: ProgressDialogType.Normal, isDismissible: false, showLogs: false);
  pr.show();
  bool userDataUploaded = false;
  AuthResult authResult;
  CollectionReference cartRef = Firestore.instance.collection('formm');

  await cartRef.document().setData({
    "no": user.no,
    "date": user.date,
    "time": user.time,
...
placeOrder(BuildContext context, double total, String nop) async {
  pr = new ProgressDialog(context,
      type: ProgressDialogType.Normal, isDismissible: false, showLogs: false);
  pr.show();
  try {
